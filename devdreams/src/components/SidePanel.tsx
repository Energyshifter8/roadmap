...
export default function SidePanel({ open, onClose, node }: Props) {
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent side="right" className="bg-zinc-900 border-zinc-700 text-white w-80">
        <SheetHeader>
          <SheetTitle className="text-white font-mono text-sm">{node?.label}</SheetTitle>
        </SheetHeader>
        <div className="mt-4 space-y-4">
          {node?.level && (
            <span className="text-xs text-zinc-400 font-mono">
              {tLevels(node.level as any) ?? node.level}
            </span>
          )}
          {node?.description ? (
            <p className="text-zinc-300 text-sm leading-relaxed">{node.description}</p>
          ) : (
            <p className="text-zinc-600 text-sm italic">No description available.</p>
          )}
          {node?.label && (
            <Button
              variant="outline"
